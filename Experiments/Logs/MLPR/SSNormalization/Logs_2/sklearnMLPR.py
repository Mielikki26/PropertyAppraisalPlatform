...
"""
MLPRegressor(scikit-learn) training script:
This script trains a MLPRegressor model on the datasets.
...
json_file = 'resultsMLPR_2.json' #json file name
fd = "\Logs\MLPR\SSNormalization\Logs_2\\" #files_directory
...
def trainxtimes(X, Y, filename,x):
    ...
    it also creates the images regarding the loss curve,
    validation score curve and predictions vs actual values
    ...
    for i in range(x):
        ...
        mlpr.fit(x_train_norm, y_train_norm.ravel())
        print("ended training at: " + str(time()))

        predictions_norm = mlpr.predict(x_test_norm).reshape(-1, 1)
        ...
        if images == True and i == 0:
            ...
            epochs = len(mlpr.loss_curve_)
            x_axis = range(0, epochs)
            fig, ax = plt.subplots()
            ax.plot(x_axis, mlpr.loss_curve_, label='Train loss')
            ax.legend()
            plt.ylabel('RMSE')
            plt.xlabel('Epochs')
            ...
            epochs = len(mlpr.validation_scores_)
            x_axis = range(0, epochs)
            fig, ax = plt.subplots()
            ax.plot(x_axis, mlpr.validation_scores_, label='Validation score')
            ax.legend()
            plt.ylabel('Mean Accuracy')
            plt.xlabel('Epochs')
            plt.title('Dataset is ' + str(filename))
            plt.savefig(cur_dir + fd + '\Images\\' + r'ValidationAcc_' + filename + '.png')
            plt.close()
...
mlpr_data = {'model': "MLPR", 'datasets': datasets_list, 'avg_mape': avg_mape, 'avg_r2': avg_r2, 'avg_mae': avg_mae}
