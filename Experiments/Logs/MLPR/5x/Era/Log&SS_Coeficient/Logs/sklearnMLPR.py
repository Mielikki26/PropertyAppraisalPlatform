...
This script trains a MLPR model on the Era Coeficient dataset.
...
fd = r"\Logs\MLPR\5x\Era\Log&SS_Coeficient\Logs\\" #files_directory
...
def trainxtimes(X_continuos, X_categorical, Y, filename,x):
    ...
    for i in range(x):
        ...
        results_df.to_csv(
            cur_dir + fd + '\Predictions' + '\iter_' + str(i) + '\\' + filename[:-4] + '_' + str(i) + '_preds.csv',
            index=False)
