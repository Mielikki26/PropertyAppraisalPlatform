...
This script trains a MLPR model on the Era Geopy dataset.
...
fd = r"\Logs\MLPR\5x\Era\Log&SS_Geopy\Logs\\" #files_directory
...
categorical_features = ['Agency', 'Property Type', 'Transaction Type', 'Status', 'Zone', 'Parish', 'County', 'Region',
                        'country', 'road', 'municipality', 'postcode']
labels = ["Price"]
...
def trainxtimes(X_continuos, X_categorical, Y, filename,x):
    ...
    for i in range(x):
        ...
        results_df = pd.DataFrame(full_array, columns=['Number of Bedrooms', 'WC', 'Latitude', 'Longitude',
                                                       ...
                                                       'Region','country', 'road', 'municipality', 'postcode',
                                                       'Price', 'Predictions'])
