...
json_file = 'resultsMLPR_logarithm.json' #json file name
fd = r"\Logs\MLPR\5x\Log Normalization\Logs\\" #files_directory
...
def trainxtimes(X, Y, filename,x):
    ...
    for i in range(x):
        ...
        x_train_log, x_test_log, y_train, y_test = train_test_split(X_log, Y, train_size=0.7, test_size=0.3, random_state=i)
        ...
        model.fit(x_train_log, y_train_log.ravel())
        print("ended training at: " + str(time()))

        predictions_log = model.predict(x_test_log).reshape(-1, 1)
