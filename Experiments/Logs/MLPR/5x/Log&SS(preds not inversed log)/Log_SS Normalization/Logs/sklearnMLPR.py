...
json_file = 'resultsMLPR_logarithmSS.json' #json file name
fd = r"\Logs\MLPR\5x\Log_SS Normalization\Logs\\" #files_directory
...
def trainxtimes(X, Y, filename,x):
    ...
    print("Normalization used is Log Transformation with SS Normalization")
    ...
    for i in range(x):
        ...
        x_train_log, x_test_log, y_train_log, y_test_log = train_test_split(X_log, Y_log, train_size=0.7, test_size=0.3, random_state=i)

        # Normalize the data
        x_train_norm = scalerx.transform(x_train_log)
        y_train_norm = scalery.transform(y_train_log)
        x_test_norm = scalerx.transform(x_test_log)
        ...
        predictions_log = scalery.inverse_transform(predictions_norm)
...
print("In this script all the datasets are used and the features are normalized using the logarithmic function and SS Normalization")
...
tabnet_data = {'model': "MLPR", 'datasets': datasets, 'avg_mape': avg_mape, 'avg_r2': avg_r2, 'avg_mae': avg_mae}
