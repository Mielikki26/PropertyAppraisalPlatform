...
This script continues the augmentation tests
by testing the DC dataset with 14 new features
...
json_file = 'resultsTabnet_MF_DC.json' #json file name
fd = r"\Logs\TabNet\Augmented\Logs_MF_DC\\" #files_directory
...
continuos_features = ["Area", "Baths", "Beds", "Latitude", "Longitude", "Month", "Year",
                      "HF_BATHRM", "AC", "NUM_UNITS", "ROOMS", "AYB", "YR_RMDL", "EYB",
                      "STORIES", "QUALIFIED", "SALE_NUM", "KITCHENS", "FIREPLACES",
                      "SOURCE", "ZIPCODE"]
labels = ["Price"]

#directories
cur_dir = os.getcwd()
datasets_dir = str(cur_dir) + r'\MoreFeatures_noSA_datasets\\'
datasets_list = os.listdir(str(cur_dir) + r'\MoreFeatures_noSA_datasets\\')

#datasets to use
dataset = "DC Properties.csv"
...
def trainxtimes(X_continuos, Y, filename, x):
    """
    ...
    # Normalize the data
    X = scalerx.transform(X_continuos)
    ...
    for i in range(x):
        ...
        print(list(zip(continuos_features, tabnet.feature_importances_)))
...
print("In this script only the DC dataset is used and its a continuation of the augmentation tests"
      "Here the test done is to test the improvement that the new 14 features provide")


df = pd.read_csv(datasets_dir + dataset, index_col=None, header=0)
df = df.sample(frac=1)
...
mapes, r2s, maes = trainxtimes(X_continuos, Y, dataset, 10)
avg_mape.append((mean(mapes), np.std(mapes)))
avg_r2.append((mean(r2s), np.std(r2s)))
avg_mae.append((mean(maes), np.std(maes)))

#Save results in json file:
tabnet_data = {'model': "TabNet", 'datasets': dataset, 'avg_mape': avg_mape, 'avg_r2': avg_r2, 'avg_mae': avg_mae}
