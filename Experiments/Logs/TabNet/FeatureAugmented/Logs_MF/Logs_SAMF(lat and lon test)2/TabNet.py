...
This script continues the latitude/longitude only tests
by adding another feature that should be obtainable from these 2 - City
...
json_file = 'resultsTabnet_SAMF(lat and lon test)2.json' #json file name
fd = r"\Logs\TabNet\Augmented\Logs_SAMF(lat and lon test)2\\" #files_directory
...
continuos_features = ["Latitude", "Longitude"]
categorical_features = ["Country", "Province", "City"]
labels = ["Price"]

#directories
cur_dir = os.getcwd()
datasets_dir = str(cur_dir) + r'\SA_datasets_2\\'
datasets_list = os.listdir(str(cur_dir) + r'\SA_datasets_2\\')

#datasets to use
datasets = ["ar properties.csv", "co properties.csv", "pe properties.csv", "uy properties.csv"]
...
print("In this script only the South American datasets and its a countinuation of the latitude/longitude test"
      "Here the test done is to test the improvement that the new related to lat/lon features provide"
      "The idea is: The new features should be reflected on the lat/lon, so does them existing provide improvements?"
      "Shouldnt the neuralnet be able to obtained them automatically?")
...
for filename in datasets_list:
    ...
    mapes, r2s, maes = trainxtimes(X_continuos, X_categorical, Y, filename, 10)
...
mapes, r2s, maes = trainxtimes(X_continuos, X_categorical, Y_all, "all_datasets", 10)
avg_mape.append((mean(mapes), np.std(mapes)))
avg_r2.append((mean(r2s), np.std(r2s)))
avg_mae.append((mean(maes), np.std(maes)))
