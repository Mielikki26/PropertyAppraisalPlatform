...
This script trains a TabNet model on the above and below 5% augmented full datasets.
...
json_file = 'resultsTabnet_geopy_AboveBelow5%.json' #json file name
fd = r"\Logs\TabNet\Augmented_AboveBelow5%\Logs\\" #files_directory
...
#directories
cur_dir = os.getcwd()
datasets_dir = str(cur_dir) + r'\AboveBelow_5%\\'
datasets_list = os.listdir(str(cur_dir) + r'\AboveBelow_5%\\')
...
for filename in datasets_list:
    ...
    try:
        mapes, r2s, maes = trainxtimes(X_continuos, X_categorical, Y, filename, 2)
...
try:
    mapes, r2s, maes = trainxtimes(X_continuos, X_categorical, Y_all, "all_datasets", 2)
    avg_mape.append((mean(mapes), np.std(mapes)))
    avg_r2.append((mean(r2s), np.std(r2s)))
    avg_mae.append((mean(maes), np.std(maes)))
except:
