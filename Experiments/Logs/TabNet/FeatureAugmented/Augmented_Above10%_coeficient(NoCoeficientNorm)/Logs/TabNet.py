...
fd = r"\Logs\TabNet\Augmented_Above10%_coeficient(NoCoeficientNorm)\Logs\\" #files_directory
...
for filename in datasets_list:
    ...
    try:
        mapes, r2s, maes = trainxtimes(X_continuos, X_categorical, Y, filename, 1)
...
try:
    mapes, r2s, maes = trainxtimes(X_continuos, X_categorical, Y_all, "all_datasets", 1)
    avg_mape.append((mean(mapes), np.std(mapes)))
    avg_r2.append((mean(r2s), np.std(r2s)))
    avg_mae.append((mean(maes), np.std(maes)))
except:
