...
This script trains a TabNet model on the Era dataset using CoefZone(NoND)
...
json_file = 'resultsTabnet_Era_CoefZone_NoND.json' #json file name
fd = r"\Logs\TabNet\Era\Logs_Era_CoefZone_NoND\\" #files_directory
...
#directories
cur_dir = os.getcwd()
datasets_dir = str(cur_dir) + r'\Era_dataset\Coeficientes\FloorArea(No ND)\\'
datasets_list = os.listdir(str(cur_dir) + r'\Era_dataset\Coeficientes\FloorArea(No ND)\\')

#datasets to use
datasets = "Era_Coef_Zone.csv"
...
def trainxtimes(X_continuos, X_categorical, Y, filename, x):
    ...
    for i in range(x):
        ...
        x_train_norm, x_test_norm, y_train, y_test = train_test_split(X, Y, train_size=0.8, test_size=0.2, random_state=i)
        x_train_norm, x_val_norm, y_train, y_val = train_test_split(x_train_norm, y_train, train_size=0.8, test_size=0.2,
                                                          random_state=i)
        ...
        predictions = tabnet.predict(x_test_norm).reshape(-1, 1)
        ...
        x_test_continuos = scalerx.inverse_transform(x_test_norm[:, 0:25])
        x_test_categorical = enc.inverse_transform(x_test_norm[:, 25:])
        ...
        results_df = pd.DataFrame(full_array, columns=['Number of Bedrooms', 'WC', 'Latitude', 'Longitude',
                                                       ...
                                                       'inquilin',
                                                       'financiamento', 'logradouro', 'R/C', 'Agency', 'Property Type',
                                                       'Transaction Type', 'Status', 'Zone', 'Parish', 'County',
                                                       'Region',
                                                       'CoefZone', 'Predictions'])
...
mapes, r2s, maes = trainxtimes(X_continuos, X_categorical, Y, datasets, 1)
