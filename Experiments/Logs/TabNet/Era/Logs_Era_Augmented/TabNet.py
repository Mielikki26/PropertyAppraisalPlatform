...
This script trains a TabNet model on the Era dataset after
...
json_file = 'resultsTabnet_Era_Augmented.json' #json file name
fd = r"\Logs\TabNet\Era\Logs_Era_Augmented\\" #files_directory
...
categorical_features = ['Agency', 'Property Type', 'Transaction Type', 'Status', 'Zone', 'Parish', 'County', 'Region',
                        'country', 'suburb', 'road', 'municipality', 'state', 'postcode']
labels = ["Price"]
...
#datasets to use
datasets = "DataBase_Era_augmented.csv"
...
def trainxtimes(X_continuos, X_categorical, Y, filename, x):
    ...
    for i in range(x):
        ...
        print("Train/val/test division is 64/16/20")
        ...
        results_df = pd.DataFrame(full_array, columns=['Number of Bedrooms', 'WC', 'Latitude', 'Longitude',
                                                       ...
                                                       'Region','country', 'suburb', 'road', 'municipality', 'state', 'postcode',
                                                       'Price', 'Predictions'])
...
print("In this script only the augmented Era dataset is used")
