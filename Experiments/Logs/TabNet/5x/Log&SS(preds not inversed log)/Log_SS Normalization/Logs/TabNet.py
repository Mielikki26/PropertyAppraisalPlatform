...
json_file = 'resultsTabnet_logarithmSS.json' #json file name
fd = r"\Logs\TabNet\5x\Log_SS Normalization\Logs\\" #files_directory
...
def trainxtimes(X, Y, filename,x):
    ...
    for i in range(x):
        ...
        x_train_log, x_test_log, y_train_log, y_test_log = train_test_split(X_log, Y_log, train_size=0.8, test_size=0.2, random_state=i)
        x_train_log, x_val_log, y_train_log, y_val_log = train_test_split(x_train_log, y_train_log, train_size=0.8, test_size=0.2, random_state=i)


        # Normalize the data
        x_train_norm = scalerx.transform(x_train_log)
        y_train_norm = scalery.transform(y_train_log)
        x_val_norm = scalerx.transform(x_val_log)
        y_val_norm = scalery.transform(y_val_log)
