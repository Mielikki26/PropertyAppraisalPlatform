...
This script trains a TabNet model on the Era dataset.
...
fd = r"\Logs\TabNet\5x\Era\Log&SS\Logs\\" #files_directory
...
def trainxtimes(X_continuos, X_categorical, Y, filename, nr_iter):
    ...
    for i in range(nr_iter):
        ...
        results_df = pd.DataFrame(full_array, columns=['Number of Bedrooms', 'WC', 'Latitude', 'Longitude',
                                                       ...
                                                       'vista', 'renovado', 'renovação', 'parqueamento',
                                                       'garagem', 'obras', 'remodelado', 'remodelação',
                                                       'com elevador', 'box', 'arrenda', 'arrendamento', 'inquilin',
        ...
        # save tabnet model
        saving_path_name = cur_dir + fd + r'\tabnet_model_iter_' + str(i)
