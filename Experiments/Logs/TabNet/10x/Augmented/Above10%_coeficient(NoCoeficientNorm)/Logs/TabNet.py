...
This script trains a TabNet model on the above 10% augmented COEFICIENT datasets with no coeficient norm.
...
json_file = 'resultsTabnet_geopy_Above10%_coeficient(NoCoeficientNorm).json' #json file name
fd = r"\Logs\TabNet\10x\Augmented\Above10%_coeficient(NoCoeficientNorm)\Logs\\" #files_directory
images = False        #flag to create images
    ...
    for i in range(x):
        ...
        if images:
            if not os.path.exists(cur_dir + fd + '\Images' + '\iter_' + str(i) + '\\'):
                os.mkdir(cur_dir + fd + '\Images' + '\iter_' + str(i) + '\\')
        ...
        if not os.path.exists(cur_dir + fd + '\Predictions' + '\iter_' + str(i) + '\\'):
            os.mkdir(cur_dir + fd + '\Predictions' + '\iter_' + str(i) + '\\')
...
for filename in datasets_list:
    ...
    try:
        ...
        print("erro no dataset: " + filename)
        avg_mape.append((0, 0))
        avg_r2.append((0, 0))
        avg_mae.append((0, 0))
...
try:
    ...
    print("erro no dataset: all_datasets")
    avg_mape.append((0, 0))
    avg_r2.append((0, 0))
    avg_mae.append((0, 0))
