...
fd = r"\Logs\TabNet\SS_Log Normalization\Logs\\" #files_directory
...
def trainxtimes(X, Y, filename,x):
    ...
    for i in range(x):
        ...
        eval_set = [(x_train_norm, y_train_log), (x_val_norm, y_val_log)]
        print("started training at: " + str(time()))
        tabnet.fit(
            X_train=x_train_norm, y_train=y_train_log,
            eval_set=eval_set,
            eval_metric=['rmse'],
            max_epochs=150,
            patience=30,
        ...
        predictions_log = tabnet.predict(x_test_norm).reshape(-1, 1)
...
print("In this script all the datasets are used and the price is normalized using the logarithmic function")
