...
json_file = 'resultsTabnet_2.json' #json file name
fd = r"\Logs\TabNet\SSNormalization\Logs_2\\" #files_directory
...
#datasets to use
datasets = ["all perth.csv", "kc house data.csv"]
...
print("In this script the smaller dataset is increased by sampling random rows and modifying them with a 1% noise"
...
for filename in datasets_list:
    ...
    if df.shape[0] < max_shape:
        for i in range(max_shape-df.shape[0]):
            ...
            value = random.randint(99, 101) / 100
            row["Price"] = row["Price"] * value
            row["Area"] = row["Area"] * value
            row["Baths"] = row["Baths"] * value
            row["Beds"] = row["Beds"] * value
            row["Latitude"] = row["Latitude"] * value
            row["Longitude"] = row["Longitude"] * value
