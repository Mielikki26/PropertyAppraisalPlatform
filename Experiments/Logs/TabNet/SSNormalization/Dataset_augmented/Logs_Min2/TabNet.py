...
json_file = 'resultsTabnet_Min2.json' #json file name
fd = r"\Logs\TabNet\SSNormalization\Logs_Min2\\" #files_directory
...
print("In this script all datasets are decreased in size down to the size of the smallest dataset by sampling "
      "random rows and deleting them from the data"
...
for filename in datasets_list:
    ...
    if df.shape[0] > min_shape:
        remove_n_rows = df.shape[0]-min_shape
        drop_indices = np.random.choice(df.index, remove_n_rows, replace=False)
