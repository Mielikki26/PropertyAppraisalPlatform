...
fd = r"\Logs\RF\5x\Log&SS\Augmented\Above10%_coeficient\Logs\\" #files_directory
...
def trainxtimes(X_continuos, X_categorical, Y, filename,x):
    ...
    for i in range(x):
        ...
        inverse_transform_lambda = lambda x: enc_dict[x.name].inverse_transform(x)
        x_test_categorical = pd.DataFrame(x_test_norm[:, 7:12].astype(int), columns=categorical_features)
        x_test_categorical = x_test_categorical.apply(inverse_transform_lambda)
        ...
        results_df = pd.DataFrame(full_array, columns=["Area", "Baths", "Beds", "Latitude",
                                                       ...
                                                       "Price", "Prediction"])
        results_df.to_csv(cur_dir + fd + '\Predictions' + '\iter_' + str(i) + '\\' + filename[:-4] + '_' + str(i) + '_preds.csv',
            index=False)
