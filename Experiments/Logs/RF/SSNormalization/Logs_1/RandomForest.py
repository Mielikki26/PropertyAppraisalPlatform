...
json_file = 'resultsRF_1.json' #json file name
fd = "\Logs\RF\SSNormalization\Logs_1\\" #files_directory
...
def trainxtimes(X, Y, filename,x):
    ...
    for i in range(x):
        ...
        predictions_norm = rf.predict(x_test_norm).reshape(-1, 1)
        ...
        print(list(zip(features, rf.feature_importances_)))
...
rf_data = {'model': "RF", 'datasets': datasets_list, 'avg_mape': avg_mape, 'avg_r2': avg_r2, 'avg_mae': avg_mae}
