...
json_file = 'resultsTabnet_Gauss(lat and lon).json' #json file name
fd = r"\Logs\TabNet\SSNormalization\Logs_Gauss(lat and lon)\\" #files_directory
...
print("In this script all datasets are increased in size up to the size of the biggest dataset by sampling "
      ...
      "Only latitude and longitude are used in training"
...
for filename in datasets_list:
    ...
    if df.shape[0] < max_shape:
        for i in range(max_shape-df.shape[0]):
            ...
            gauss_noise = round(random.gauss(0, (row["Latitude"] - mean_lat) / std_lat) * 0.01, 5)
            row["Latitude"] = np.float64(row["Latitude"] + gauss_noise * row["Latitude"])

            gauss_noise = round(random.gauss(0, (row["Longitude"] - mean_lon) / std_lon) * 0.01, 5)
            row["Longitude"] = np.float64(row["Longitude"] + gauss_noise * row["Longitude"])
