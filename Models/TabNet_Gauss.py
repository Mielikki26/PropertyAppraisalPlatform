...
json_file = 'resultsTabnet_Gauss.json' #json file name
fd = r"\Logs\TabNet\SSNormalization\Logs_Gauss\\" #files_directory
...
print("In this script all datasets are increased in size up to the size of the biggest dataset by sampling "
      "random rows and modifying them with a noise depending on the standard deviation of the value in question"
      "This is done to test the possibility that the variance in datasets sizes is decreasing performance"
      "By evening out the sizes its excepted that the model achieves better performance")
...
for filename in datasets_list:
    ...
    mean_area = df["Area"].median()
    std_area = df["Area"].std()

    mean_baths = df["Baths"].median()
    std_baths = df["Baths"].std()

    mean_beds = df["Beds"].median()
    std_beds = df["Beds"].std()
    ...
    if df.shape[0] < max_shape:
        for i in range(max_shape-df.shape[0]):
            row = df.sample(random_state=i)

            gauss_noise = random.gauss(0, (row["Area"]-mean_area)/std_area)*0.01
            row["Area"] = row["Area"] + gauss_noise*row["Area"]

            gauss_noise = random.gauss(0, (row["Baths"] - mean_baths) / std_baths) * 0.01
            row["Baths"] = row["Baths"] + gauss_noise * row["Baths"]

            gauss_noise = random.gauss(0, (row["Beds"] - mean_beds) / std_beds) * 0.01
            row["Beds"] = row["Beds"] + gauss_noise * row["Beds"]

            gauss_noise = random.gauss(0, (row["Latitude"] - mean_lat) / std_lat) * 0.01
            row["Latitude"] = row["Latitude"] + gauss_noise * row["Latitude"]

            gauss_noise = random.gauss(0, (row["Longitude"] - mean_lon) / std_lon) * 0.01
            row["Longitude"] = row["Longitude"] + gauss_noise * row["Longitude"]
    ...
    mapes, r2s, maes = trainxtimes(X, Y, filename, 2)
...
mapes, r2s, maes = trainxtimes(X_all, Y_all, "all_datasets", 2)
avg_mape.append((mean(mapes), np.std(mapes)))
avg_r2.append((mean(r2s), np.std(r2s)))
avg_mae.append((mean(maes), np.std(maes)))
